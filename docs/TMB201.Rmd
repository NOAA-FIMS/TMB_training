...
* TMB's **oneStepPredict()** function calculates quantile residuals

.pull-left[<br>
$\Large r_{i} = \phi^{-1}(F(Y_{i},\Theta))$<br>
...
```
* The cdf and oneStepGeneric methods are the only methods available for discrete data
* OSA residuals are challenging to calculate when data are mixed (eg. delta-lognormal, tweedie)
